...
function formatFilm(film) {
  ...
  <div class="ml-48 p-2">
    ...
    <div><span class="font-bold w-24 inline-block">Passed:</span>${
      film.passed
    }</div>
    <div><span class="font-bold w-24 inline-block">Reason:</span> ${
      film.reason
    }</div>
    <div><span class="font-bold w-24 inline-block">Budget:</span> $${film.budget.toLocaleString(
      "en-US"
    )}</div>
    <div><span class="font-bold w-24 inline-block">Dom. Gross:</span> $${film.domesticGross.toLocaleString(
      "en-US"
    )}</div>
    <div class="info"> <span class="font-bold w-24 inline-block">Int'l Gross:</span>  $${film.internationalGross.toLocaleString(
      "en-US"
    )}</div>
    <p class="text-xs line-clamp-2">${film.overview}</p>
    </div>
  </div>`;
}
